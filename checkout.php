...
<section class="checkout-orders">
   ...
   <h3>Your Orders</h3>
      ...
      <?php
         $grand_total = 0;
         $cart_items = []; // fixed initialization
      ...
      ?>
         <p style="color:black ;"> <?= $fetch_cart['name']; ?> <span>(<?= 'Nrs. '.$fetch_cart['price'].'/- x '. $fetch_cart['quantity']; ?>)</span> </p>
      <?php
            }
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; gap: 20px; margin-top:1rem;">
         <input type="submit" name="order" class="btn <?= ($grand_total > 1)?'':'disabled'; ?>" value="place order">
         <button type="button" id="khaltiPayBtn" class="btn">Pay with Khalti</button>
      </div>
...
<script>
   const khaltiConfig = {
      publicKey: "test_public_key_dc74e7cdecf94f88b1911f2e3c1c3f22", // Replace with your Khalti public key
      ...
      eventHandler: {
         onSuccess(payload) {
            ...
            // Append Khalti payload data
            fetch('verify_khalti_payment.php', {
               method: 'POST',
   ...
   document.getElementById('khaltiPayBtn').addEventListener('click', () => {
      const totalPrice = parseInt(document.querySelector('input[name="total_price"]').value);
      if(totalPrice > 0){
         khaltiCheckout.show({amount: totalPrice * 100}); // amount in paisa
      } else {
